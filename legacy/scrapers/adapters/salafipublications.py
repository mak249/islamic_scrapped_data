"""
salafipublications.com adapter - Metadata-only scraping (title, URL, date, author, description).
"""
...
class SalafiPublicationsAdapter(BaseScraper):
    """
    Adapter for salafipublications.com - metadata only (respects copyright).
    """
    
    def __init__(self, start_urls: Optional[List[str]] = None):
        """
        Initialize salafipublications.com adapter.
        
        Args:
            start_urls: List of starting URLs
        """
        super().__init__(
            source_name='salafipublications',
            base_url='https://salafipublications.com'
        )
        self.start_urls_list = start_urls or ["https://salafipublications.com"]
    ...
    def parse(self, response) -> Optional[Dict[str, Any]]:
        """
        Parse salafipublications.com page - extract metadata only.
        ...
        return {
            'id': f"salafipublications_{hash(url)}",
            'url': url,
