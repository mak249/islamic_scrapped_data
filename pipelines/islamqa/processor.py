...
Examples:
  ...
  python clean_and_separate.py --db data.db
  ...
  python clean_and_separate.py --all --output clean_rag_data
    ...
    if args.all:
        # Scan root and legacy/databases
        db_files = [f for f in os.listdir('.') if f.endswith('.db')]
        
        legacy_path = Path('../../legacy/databases')
        if legacy_path.exists():
            legacy_dbs = [str(legacy_path / f) for f in os.listdir(legacy_path) if f.endswith('.db')]
            db_files.extend(legacy_dbs)
        ...
        db_files = ['data.db']
        if os.path.exists('islamic_data.db'):
            db_files.append('islamic_data.db')
